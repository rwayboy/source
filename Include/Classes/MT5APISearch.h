...
public:
   ...
   //--- quick sort
   template <class T>
   static void       QuickSort(T *base,uint32_t num,SortFunctionPtr compare);
   //---
...
//+------------------------------------------------------------------+
template<class T>
inline void SMTSearch::QuickSort(T *base,uint32_t num,SortFunctionPtr compare)
  {
   ...
   T *lostk[30], *histk[30];
   int32_t stkptr;              /* stack for saving sub-array to be processed */
//---
   if(num < 2) return;
