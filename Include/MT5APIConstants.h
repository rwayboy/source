...
//--- config management retcodes
   ...
   MT_RET_CFG_INVALID_COMPANY    =2021,   // Company name does not belong to a license or whitelabel
//--- client management retcodes
   ...
   MT_RET_USR_ACCOUNT_EXIST     =3013,    // Account already exists
...
//--- Subscriptions retcodes
   ...
   MT_RET_SUBS_PAYMENT_METHOD          =15008, // Unsupported payment method
//--- Payments
   MT_RET_PAY_REAL_ONLY                =16000,  // only for real accounts
   MT_RET_PAY_INVALID_AMOUNT           =16001,  // invalid payment ammount
   MT_RET_PAY_NOT_ALLOWED_DEPOSIT      =16002,  // deposit operation isn't allowed
   MT_RET_PAY_NOT_ALLOWED_WITHDRAWAL   =16003,  // withdrawal operation isn't allowed
   MT_RET_PAY_NOT_ALLOWED_GROUP        =16004,  // account group isn't allowed
   MT_RET_PAY_NOT_ALLOWED_COUNTRY      =16005,  // account country isn't allowed
   MT_RET_PAY_DECLINE_BY_RULES         =16006,  // payment rejected by payment rules
   MT_RET_PAY_DECLINE_BY_AML           =16007,  // payment rejected by payment AML check
   MT_RET_PAY_LIMIT_DEPOSIT_MIN        =16008,  // deposit ammount less than allowed
   MT_RET_PAY_LIMIT_DEPOSIT_MAX        =16009,  // deposit ammount great than allowed
   MT_RET_PAY_LIMIT_WITHDRAWAL_MIN     =16010,  // withdrawal ammount less than allowed
   MT_RET_PAY_LIMIT_WITHDRAWAL_MAX     =16011,  // withdrawal ammount great than allowed
   MT_RET_PAY_PROVIDER_PAYMENT         =16012,  // payment provider process error
   MT_RET_PAY_PROVIDER_STATUS          =16013,  // payment provider status request error
   MT_RET_PAY_CONVERSION               =16014,  // payment currency conversion error
   MT_RET_PAY_NOT_WAITING              =16015,  // payment isn't waiting verification
   MT_RET_PAY_VERIFICATION             =16016,  // payment isn't passed verification
   MT_RET_PAY_INVOICE                  =16017,  // is should be called Invoive method to receive invoice
   MT_RET_PAY_INVALID_CURRENCY         =16018,  // wrong payment currency
   MT_RET_PAY_LIMIT_REACHED            =16019,  // payment limit reached
   MT_RET_PAY_PROVIDER_REFUND          =16020,  // payment provider process error
   MT_RET_PAY_DECLINE_BY_CARDHOLDER_NAME=16021   // payment rejected by cardholder name verification
  };
//+------------------------------------------------------------------+
