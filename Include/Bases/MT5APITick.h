...
struct MTTick
  {
   ...
   int64_t           datetime;                        // datetime in seconds since 01/01/1970
   //--- basic price data
   double            bid;                             // bid price
   double            ask;                             // ask price
   double            last;                            // last price
   uint64_t          volume;                          // last trade volume
   //--- additional data
   int64_t           datetime_msc;                    // datetime in milliseconds since 01/01/1970
   uint64_t          flags;                           // flags
   uint64_t          volume_ext;                      // last trade volume with extended accuracy
   //--- reserved data
...
struct MTTickShort
  {
   ...
   int64_t           datetime;                        // last update datetime in seconds since 01/01/1970
   double            bid;                             // bid price
   double            ask;                             // ask price
   double            last;                            // last price
   uint64_t          volume;                          // last trade volume
   int64_t           datetime_msc;                    // datetime in milliseconds since 01/01/1970
   uint64_t          flags;                           // flags
   uint64_t          volume_ext;                      // last trade volume with extended volume
   uint32_t          reserved[26];                    // reserved
  };
#pragma pack(pop)
//+------------------------------------------------------------------+
...
struct MTTickStat
  {
   ...
   //--- trade volume
   uint64_t          vol_high;                     // last high
   uint64_t          vol_low;                      // last low
   //--- trade session  statistics
   uint64_t          trade_deals;                  // total deals in current session
   uint64_t          trade_volume;                 // total deals volume in current session
   uint64_t          trade_turnover;               // total turnover in current session
   uint64_t          trade_interest;               // open interest in current session
   uint64_t          trade_buy_orders;             // total buy orders
   uint64_t          trade_buy_volume;             // total buy orders volume
   uint64_t          trade_sell_orders;            // total sell orders
   uint64_t          trade_sell_volume;            // total sell orders volume
   uint64_t          trade_volume_ext;             // total deals volume in current session with extended accuracy
   uint64_t          trade_buy_volume_ext;         // total buy orders volume with extended accuracy
   uint64_t          trade_sell_volume_ext;        // total sell orders volume with extended accuracy
   uint64_t          vol_high_ext;                 // last high with extended accuracy
   uint64_t          vol_low_ext;                  // last low with extended accuracy
   int32_t           trade_reserved[20];           // reserved
   //--- datetime
   int64_t           datetime_msc;                 // last update datetime in milliseconds since 01/01/1970
   ...
   int32_t           price_reserved[14];           // reserved
  };
#pragma pack(pop)
//+------------------------------------------------------------------+
...
public:
   ...
   //--- tick hooks
   virtual MTAPIRES  HookTick(const int32_t /*feeder*/,MTTick& /*tick*/)              { return(MT_RET_OK); }
   virtual MTAPIRES  HookTickStat(const int32_t /*feeder*/,MTTickStat& /*tstat*/)     { return(MT_RET_OK); }
   //--- extended tick events
   virtual void      OnTick(const int32_t /*feeder*/,const MTTick& /*tick*/)          { }
   virtual void      OnTickStat(const int32_t /*feeder*/,const MTTickStat& /*tstat*/) { }
  };
//+------------------------------------------------------------------+
